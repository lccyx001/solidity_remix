...
contract BlindAuction {
    ...
    // 函数修改器 https://learnblockchain.cn/docs/solidity/contracts.html#functions
        ...
        onlyBefore(revealEnd)
    {
        ...
        for (uint i = 0; i < length; i++) {
            ...
            (uint value, bool fake_, bytes32 secret_) =
                    (values[i], fake[i], secret[i]);
                    //abi.encodePacked(arg);
            if (bid_.blindedBid != keccak256(abi.encodePacked(value, fake_, secret_))) {
                // 出价未能正确披露
                // 不返还订金
                continue;
            }
            refund += bid_.deposit;
            if (!fake_ && bid_.deposit >= value) {
